...
class CausalMultiHeadSelfAttention(nn.Module):
    def __init__(self, d_model, num_heads, rope_theta=10000.0, max_seq_len=2048,device=None, dtype=None):
        super().__init__()
    ...
    def forward(self, x: torch.Tensor, pos_ids: torch.Tensor | None = None) -> torch.Tensor:
        ...
        # 2. Reshape for Multi-Head (B, T, D) -> (B, H, T, d_head)
        # Assuming d_model=64 and n_heads=4, d_head=16
        b, t, d = x.shape
        ...
        # 4. Create Causal Mask (fixes numerical mismatches)
        # Masks the upper triangle (future tokens)
        mask = torch.triu(torch.ones(t, t, device=x.device), diagonal=1).bool()

        # 5. Scaled Dot Product Attention
        # Ensure your scaled_dot_product_attention handles the mask
        attn_out = scaled_dot_product_attention(q, k, v, mask=mask)

        # 6. Recombine Heads and Final Projection
...
class TransformerBlock(nn.Module):
    def __init__(self, d_model: int, num_heads: int, d_ff: int,max_seq_len: int = 2048, device=None, dtype=None):
        super().__init__()
