...
class Linear(nn.Module):
    def __init__(self, in_features: int, out_features: int, device=None, dtype=None):
        ...
        # Initialization: Truncated Normal N(0, 2 / (d_in + d_out)) [-3sigma, 3sigma]
...
class Embedding(nn.Module):
    def __init__(self, num_embeddings: int, embedding_dim: int, device=None, dtype=None):
        ...
        # Initialization: Truncated Normal N(0, 1) [-3, 3]
...
class RotaryPositionalEmbedding(nn.Module):
    ...
    def forward(self, x: torch.Tensor, token_positions: torch.Tensor) -> torch.Tensor:
        ...
        # View x as pairs: (..., seq_len, d_k/2, 2)
...
class CausalMultiHeadSelfAttention(nn.Module):
    ...
    def forward(self, x: torch.Tensor, pos_ids: torch.Tensor = None) -> torch.Tensor:
        ...
        if self.rope is not None and pos_ids is not None:
            # Broadcast pos_ids from (B, S) to (B, heads, S)
